...
class ButtonGroupController extends ElementController {
  ...
  // TODO: change this to use updateState(..., false) instead
  setInitialState() {
    ...
    if (selectionLimit > -1 && activeIds.length >= selectionLimit && selected) {
      if (unselectLastChoice) {
        ...
        if (unselectedButtonId !== buttonId) {
          ...
          unselectedButtonData.controller.state[ButtonGroupStates.Selected] = false;
          
          fireOnUnselect(unselectedButtonData);
          updateActiveIds(unselectedButtonId, unselectedButtonData.controller.state[ButtonGroupStates.Selected]);
        }
      } else {
