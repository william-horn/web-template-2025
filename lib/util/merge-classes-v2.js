...
export const mergeClasses = (baseClass, importedClass, customSpecialKeys={}) => {
  ...
  // if baseClass is not an object, throw error
  if (!baseClassIsObject) throw new Error("Base class must be an object or string")
  ...
  // case when: mergeClasses({ self: "" }, "string")
  if (importedClassIsString) {
    finalClass.self = twMerge(finalClass.self, importedClass)

  } else if (!importedClassIsObject) {
    throw new Error("Imported class must be an object or string")
  }
