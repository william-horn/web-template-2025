...
export const mergeClasses = (baseClass={}, importedClass={}, customSpecialKeys={}) => {
  ...
  if (!(baseClassIsObject || baseClassIsString)) throw Error("Invalid base class")
  if (!(importedClassIsObject || importedClassIsString)) throw Error("Invalid base class")

  if (baseClassIsString && importedClassIsString) return { self: twMerge(baseClass, importedClass) }
  if (baseClassIsString && importedClassIsObject) return { ...importedClass, self: twMerge(baseClass, importedClass.self) }
  if (baseClassIsObject && importedClassIsString) return { ...baseClass, self: twMerge(baseClass.self, importedClass) }
...
export const mergeStates = (baseStates=[], importedStates=[]) => {
  ...
  for (let index = 0; index < importedStates.length; index++) {
    ...
    if (baseStateMap.has(importedStateName)) {
      finalState[finalState.length] = [
        importedStateName, 
        mergeClasses(baseStateMap.get(importedStateName), importedStateClass)
      ]
    } else {
      finalState[finalState.length] = [importedStateName, importedStateClass || {}]
    }

    baseStateMap.delete(importedStateName)
  }

  return [...finalState, ...baseStateMap.entries()]
}
